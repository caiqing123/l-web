...
const getSlot = (cls = []) => {
  ...
  cls.map((item) => {
    if (item.children && item.children.length > 0) {
      const tmp = getSlot(item.children);
      sls.push(...tmp);
...
const requestData = async () => {
  ...
  if (defaultCrud.value.showIndex && columns.value.length > 0 && columns.value[0].dataIndex !== '__index') {
    columns.value.unshift({
      title: defaultCrud.value.indexLabel, dataIndex: '__index', width: 70, fixed: 'left',
    });
  }
  if (defaultCrud.value.operationColumn && columns.value.length > 0 && columns.value[columns.value.length - 1].dataIndex !== '__operation') {
    columns.value.push({
      title: defaultCrud.value.operationColumnText, dataIndex: '__operation', width: defaultCrud.value.operationWidth, align: 'right', fixed: 'right',
    });
  }
...
const searchHandler = (formData) => {
  if (settingProps.crud.requestParamsLabel && requestParams.value[settingProps.crud.requestParamsLabel]) {
    ...
    requestParams.value[settingProps.crud.requestParamsLabel] = { ...formData };
  } else {
...
const dbClickOpenEdit = (record) => {
  if (defaultCrud.value.isDbClickEdit) {
    ...
    if (_.isArray(defaultCrud.value.edit.auth)) {
      for (const index in defaultCrud.value.edit.auth) {
...
const exportAction = () => {
  ...
  const download = (url) => request({
    url, data, method: 'post', timeout: 60 * 1000, responseType: 'blob',
  });
...
const __summary = ({ data }) => {
  if (defaultCrud.value.showSummary && isArray(defaultCrud.value.summary)) {
    const { summary } = defaultCrud.value;
    const summaryData = {};
