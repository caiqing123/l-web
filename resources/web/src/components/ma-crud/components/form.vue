...
const submit = async (done) => crudForm.value.validate()
  .then(async (result) => {
    if (result) {
      done(false);
      return false;
...
const edit = (data) => {
  ...
  for (const i in data) form.value[i] = data[i];
  open();
};
const requestDict = (url, method, params, data, timeout = 10 * 1000) => request({
  url, method, params, data, timeout,
});
const init = () => {
...
const handlerCascader = (val, column) => {
  if (column.cascaderItem && isArray(column.cascaderItem) && column.cascaderItem.length > 0 && val) {
    ...
    column.cascaderItem.map(async (name) => {
      const { dict } = columns.value.filter((col) => col.dataIndex === name && col.dict)[0];
