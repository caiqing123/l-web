...
const getTagColor = (row, record) => props.searchRef.dictColors(row.dataIndex, (row.dataIndex.indexOf('.') > -1) ? get(record, row.dataIndex) : record[row.dataIndex]);

const getDataIndex = (row, record) => props.searchRef.dictTrans(row.dataIndex, (row.dataIndex.indexOf('.') > -1) ? get(record, row.dataIndex) : record[row.dataIndex]);
